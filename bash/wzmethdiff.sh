...
decho "There are "$(grep 'hyper' $contrast.cpgi.diff)" statistically significant hyper-methylated CpG island and "$(grep 'hypo' $contrast.cpgi.diff)" hypo-methylated CpG island."
...
decho "There are "$(awk '$NF<50' $contrast.hyper.cgi.bed | wc -l)" CpGIs very close to hyper-methylated segments".
