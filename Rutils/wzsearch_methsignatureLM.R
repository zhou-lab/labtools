...
cat('chrm','beg','end','range','mostU','gapU',group_alphabet,'\n', sep="\t")
con <- OpenRead(args[2])
while (TRUE) {
...
#cat(table(g2),'\n')
#cat(names(table(g2)),'\n')
  tg <- g2 %in% names(which(table(g2) >=5)) # min data points
  b2 = b2[tg]
  g2 = g2[tg]

  g2levels <- sort(unique(g2))
  if(length(unique(g2)) < 15) { next; }
  ...
  cat(fields[1],deque_loc[[deque_ind]],fields[3],rg,names(sft)[1],sft[2]-sft[1],ft[group_alphabet],'\n', sep="\t")
}
