...
static inline RefSeq* init_RefSeq(char *ref_fn, uint32_t flank1, uint32_t flank2) {
	
	RefSeq *rs = calloc(1, sizeof(RefSeq));
...
static inline void fetch_RefSeq(RefSeq *rs, char *chrm, uint32_t beg, uint32_t end) {
	...
	if (rs->chrm != 0
		...
		rs->chrm = realloc(rs->chrm, strlen(chrm));
		strcpy(rs->chrm, chrm);
...
static inline char getbase_RefSeq(RefSeq *rs, uint32_t rpos) {
